...
static bool ra_get_addr_eui64(void)
{
	struct ifreq ifr;
	int sock;
...
static bool ra_get_addr_ll(void)
{
...
static bool ra_get_addr_rand(void)
{
...
static void ra_get_addr(void)
{
	bool addr_lla = false;

	switch (ra_ifid_mode) {
	case RA_IFID_EUI64:
		addr_lla |= !ra_get_addr_eui64();
		break;
	case RA_IFID_FIXED:
		/* nothing to do */
		break;
